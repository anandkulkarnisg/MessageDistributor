...
template<typename T> class MessageDistributor
{
	private:
	...
	public:
		MessageDistributor(const MessageDistributor&) = delete;				// Copy deleted.
		MessageDistributor& operator=(const MessageDistributor&) = delete;  // Assignment operator deleted.
